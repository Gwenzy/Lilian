package fr.thaksbots.base.commands;

import fr.thaksbots.base.Base;
import fr.thaksbots.base.Credentials;
import fr.thaksbots.base.exceptions.exceptions.*;
...
public abstract class Command implements IListener<MessageReceivedEvent>{
    ...
    public boolean canBeExecuted(MessageReceivedEvent event) throws ThaksbotException {
        ...
        if(!enabled && event.getAuthor().getStringID().equals(Credentials.ID_DEVELOPPER))
            enabledOk = true;
        ...
        if(prefixOk) {
            ...
            if(!this.needVoice)
                ...
                voiceOk = Base.logged.getOurUser().getVoiceStateForGuild(event.getGuild()).getChannel()!=null;
